...
export const handlePlayerKeyboard = (event, handlers) => {
	const { onPlayPause, onSeek, onVolumeUp, onVolumeDown, onNext, onPrevious, onSkipForward, onSkipBackward } = handlers;
	...
	switch (event.key) {
		...
		case 'End':
			event.preventDefault();
			if (onSeek) onSeek('end');
			break;

		case 'PageUp':
			event.preventDefault();
			if (onSeek) onSeek('forward-30');
			break;

		case 'PageDown':
			event.preventDefault();
			if (onSeek) onSeek('backward-30');
			break;
 ...
 * 
 * @returns {string} Formatted keyboard shortcuts help
 */
