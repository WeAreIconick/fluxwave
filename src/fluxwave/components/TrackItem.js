 ...
 * @return {JSX.Element} The track item component
 * @since 0.1.0
 */
const TrackItem = ( {
	track,
		...
		<div
			...
			onKeyDown={ ( e ) => {
				...
				if ( e.key === 'ArrowUp' && ! isFirst ) {
					...
					e.preventDefault();
					e.stopPropagation();
					// For now, we'll remove without confirmation to avoid using window.confirm
		...
		>
			<div className="flex items-center gap-3 p-3">
				...
				<div
					...
					role="group"
					aria-label={ __( 'Track reorder controls', 'fluxwave' ) }
				>
					<Button
						icon={ ArrowUpIcon }
						size="small"
						...
						label={
							isFirst
								? __(
										'Cannot move up - first track',
										'fluxwave'
								  )
								: __( 'Move track up', 'fluxwave' )
						}
						showTooltip={ true }
					...
					<Button
						...
						label={
							isLast
								? __(
										'Cannot move down - last track',
										'fluxwave'
								  )
								: __( 'Move track down', 'fluxwave' )
						}
						showTooltip={ true }
						className="!min-w-0 !w-11 !h-11"
						aria-describedby={ `track-${ track.id }-instructions` }
				...
				<div
					id={ `track-${ track.id }-instructions` }
					className="sr-only"
				>
					{ __(
						'Use arrow keys to reorder tracks, Enter or Space to select, Delete to remove',
						'fluxwave'
					) }
				</div>
							...
							<div className="text-center">
								...
								>
									{ track.artwork ? (
										...
										<div className="w-16 h-16 rounded bg-gradient-to-br from-blue-100 to-indigo-200 border-2 border-dashed border-blue-300 flex flex-col items-center justify-center text-blue-500 hover:border-indigo-400 hover:text-indigo-600 hover:bg-gradient-to-br hover:from-indigo-50 hover:to-blue-50 transition-all">
											<svg
